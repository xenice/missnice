...
class Guest extends Base
{
	...
	{
		parent::__construct();
		add_filter( 'the_password_form', [$this,'changePasswordForm']);
		$this->loginScripts();
    ...
    public function changePasswordForm( $post = 0 ) {
    	$post   = get_post( $post );
    	$label  = 'pwbox-' . ( empty( $post->ID ) ? rand() : $post->ID );
    	$output = '<div style = "max-width:400px;padding:15px;background-color:#fff;" ><p>' . __( 'This content is password protected. To view it please enter your password below:' ) . '</p>
    	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
    	<div class="form-group"><label for="' . $label . '">' . __( 'Password:' ) . '</label> <input class="form-control" name="post_password" id="' . $label . '" type="password" size="20" /></div><input class="btn btn-custom" type="submit" name="Submit" value="' . esc_attr_x( 'Enter', 'post password form' ) . '" /></form></div>
    	';
