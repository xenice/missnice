	...
	private function enter(){
		if(is_user_logged_in()){
			...
			if(isset($_SESSION['openid'])){
				...
				$user_ID = $wpdb->get_var("SELECT ID FROM $wpdb->users WHERE user_qq='".esc_sql($_SESSION['openid'])."'");
				//$user_ID = $wpdb->get_var("SELECT user_id FROM $wpdb->usermeta WHERE meta_key='qq' meta_value='".esc_sql($_SESSION['openid'])."'");
				if($user_ID > 0){
					...
					if ( is_wp_error( $user_id ) ) {
						...
						$ff = $wpdb->query("UPDATE $wpdb->users SET user_qq = '".esc_sql($_SESSION['openid'])."' WHERE ID = '$user_id'");
						if ($ff) {
	...
	private function bind(){
		if(is_user_logged_in()){
			if(isset($_SESSION['openid'])){
				...
				global $wpdb;
				$hasuser_ID = $wpdb->get_var("SELECT ID FROM $wpdb->users WHERE user_qq='".esc_sql($_SESSION['openid'])."'");
				//$hasuser_ID = $wpdb->get_var("SELECT user_id FROM $wpdb->usermeta WHERE meta_key='qq' meta_value='".esc_sql($_SESSION['openid'])."'");
				if($hasuser_ID){
					...
					$wpdb->query("UPDATE $wpdb->users SET user_qq = '".esc_sql($_SESSION['openid'])."' WHERE ID = $userid");
					$uinfo = json_decode($this->get_user_info());
